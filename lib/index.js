    ...
    authenticate: function (request, reply) {
      ...
      jwt.verify(token, settings.key, function(err, decoded) {
        ...
        if(err && err.message === 'jwt expired') {
          boomResp = Boom.unauthorized('Expired token received for JSON Web Token validation', 'Bearer');
        } else if (err) {
          boomResp = Boom.unauthorized('Invalid signature received for JSON Web Token validation', 'Bearer');
        }

        if (!settings.validateFunc) {
          if(boomResp){
            return reply.continue(boomResp);
          }
        ...
        settings.validateFunc(decoded, token,boomResp, function (err, isValid, credentials) {
